...
export default async function toMetric(node) {
  const [
    timeData,
    memData,
    currentLoadData,
...
function addTimeData(data) {
  return [
    ...
    add("node_local_time", data.current, "Node time", TYPE_COUNTER, {
...
function addCpuTemp(data) {
  return [add("node_cpu_temp", data.main)];
}
...
function addFs(sizes, stats, names = "*") {
  ...
  const sizesMetrics = sizes
    .filter(filterByName)
    .map((item) => [
      add("node_fs_size_bytes", item.size, "Size in bytes", TYPE_COUNTER, {
        name: item.fs,
        type: item.type,
        mount: item.mount,
      }),
      add("node_fs_used_bytes", item.used, "Used in bytes", TYPE_GAUGE, {
        name: item.fs,
        type: item.type,
        mount: item.mount,
      }),
      add("node_fs_used_percent", item.use, "Used in percent", TYPE_GAUGE, {
        name: item.fs,
        type: item.type,
        mount: item.mount,
      }),
...
function addNetwork(stats) {
  return flatArray(
    stats
      ...
      .map((stat) => [
        add("node_network_rx", stat.rx_sec, "", TYPE_GAUGE, {
          name: stat.iface,
          state: stat.operstate,
        }),
        add("node_network_tx", stat.tx_sec, "", TYPE_GAUGE, {
          name: stat.iface,
          state: stat.operstate,
        }),
      ])
...
const flatArray = (items) =>
  items.reduce((prev, current) => prev.concat(current), []);
